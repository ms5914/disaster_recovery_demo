...
class DistributedDatabase:
    ...
    def create_checkpoint(self):
        ...
        with self.data_lock:
            try:
                ...
                app.logger.error(f"The data is {self.data}")
                data_hash = self._calculate_data_hash()
                app.logger.error(f"The hash is {data_hash}")
                ...
                app.logger.error(f"Existing checkpoints {existing_checkpoints}")
                ...
                app.logger.error(f"Checkpoint file {checkpoint_file}")
            ...
            # Create the checkpoint file
                ...
                app.logger.error(f"Checkpoint data {checkpoint_data}")
                ...
                app.logger.error(f"Current checkpoint finished{checkpoint_file}")


            # Keep only the last 5 checkpoints
                self._cleanup_old_checkpoints()

                app.logger.error(f"Cleanup finished")
    ...
    def _cleanup_old_checkpoints(self):
        app.logger.error(f"Cleaning old  checkpoints")
        checkpoints = sorted([
            f for f in os.listdir(CHECKPOINT_DIR)
            if f.startswith('checkpoint_')
        ])
        app.logger.error(f"old  checkpoints filtered {checkpoints}")
